...
const RantanganList = () => {
  ...
  const fetchSellers = useCallback(async () => {
    try {
      ...
      if (response.ok && data.sellers) {
        ...
        // Filter sellers that have rantangan packages with valid data
        const sellersWithRantangan = data.sellers.filter(seller => {
          const hasRantanganPackages = seller.rantanganPackages && 
                 Array.isArray(seller.rantanganPackages) && 
                 seller.rantanganPackages.length > 0 &&
                 seller.rantanganPackages.some(pkg => 
                   pkg.name && pkg.name !== 'isi disini' && 
                   pkg.description && pkg.description !== 'isi disini' && 
                   pkg.price && pkg.price > 0
                 );
          
          return hasRantanganPackages;
        });

        console.log("Sellers with rantangan packages:", sellersWithRantangan.length);
        
        const formattedStores = sellersWithRantangan.map(seller => {
          // Handle distance formatting
          let distance = "N/A";
          
          if (seller.distance !== null && seller.distance !== undefined && 
              typeof seller.distance === 'number' && !isNaN(seller.distance)) {
        ...
        Alert.alert("Error", "Failed to load rantangan data");
      }
    } catch (error) {
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyTitle}>No Rantangan Found</Text>
